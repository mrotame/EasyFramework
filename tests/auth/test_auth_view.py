...
class TestAuthView(TestCase):
    ...
    def getUserModel(self, flask_app: Flask):
        flask_app = self.get_flask_app_sql()
        return flask_app.config.get('EASY_FRAMEWORK_USER_MODEL')

    def test_request_login_without_registered_user_and_get_invalidCredentialsException(self):
        with self.get_flask_app_sql().test_request_context(self.url(), json=self.authJson()):
    ...
    def test_request_login_and_get_token(self):
        ...
        with flask_app.test_request_context():
            self.getUserModel(flask_app)(login='test', password='test').save()
