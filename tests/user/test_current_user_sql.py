...
from easy_framework.user import AnonymousUser, UserMixin, current_user
...
class TestCurrent_user(TestCase):
    ...
    def test_request_with_existent_session_token_and_get_access_to_current_user_returning_an_userMixin_instance(
        self,
    ):
        with self.get_flask_app_sql().test_request_context("/", json=self.authJson()):
            user = self.userModel(login="test", password="test")
            user.save()
    ...
    def test_request_with_non_existent_session_token_and_get_access_to_current_user_returning_an_Anonymous_instance(
        self,
    ):
        with self.get_flask_app_sql().test_request_context("/", json=self.authJson()):
            user = self.userModel(login="test", password="test")
