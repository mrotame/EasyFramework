...
class TestBaseSerializerMongo(TestCase):
    def getMethodBasedSerializer(self):
        return SerializerTestMongo()
    ...
    def test_load_post_serializer_with_full_json_and_get_no_errors(self):
        with self.get_flask_app_mongo().test_request_context('/', json=self.post_json, method='POST'):
    ...
    def test_load_post_serializer_with_partial_json_and_get_no_errors(self):
        ...
        del json['age']
        del json['name']
        with self.get_flask_app_mongo().test_request_context('/', json=json, method='POST'):
            serializer = self.getMethodBasedSerializer()
            serializer.load(request.json)
    ...
    def test_load_update_serializer_without_required_field_and_get_no_errors(self):
        with self.get_flask_app_mongo().test_request_context('/', json=self.patch_json, method='PATCH'):
            serializer = self.getMethodBasedSerializer()
            serializer.load(request.json)
