...
class TestGenericApiViewMongo(TestCase):
        ...
        with Flask(__name__).test_request_context(json=self.genericJson()):
            ...
            try:
                self.flaskApp.add_url_rule(
                    self.view.routes[0], view_func=self.view.as_view("GenericViewTest")
                )
    ...
    def genericJson(self):
        return {
            "username": "testing",
            "password": "123",
            "desc": "Testing Easy Framework with pytest",
        }
    ...
    def test_if_view_auto_insert_data_into_database_with_post(self):
        with self.flaskApp.test_request_context(json=self.genericJson()):
            self.view().auto_post()
            self.assertTrue(
                ModelTestMongo.get_one(
                    **{"username": self.genericJson()["username"]}
                ).desc
    ...
    def test_if_view_auto_read_data_from_database_with_get(self):
        ...
        with self.flaskApp.test_request_context(
            query_string={"username": self.genericJson()["username"]}
        ):
    ...
    def test_if_getOwnedEntities_return_all_owned_entities_from_db(self):
        ...
        with self.flaskApp.app_context():
            testModels_user1 = [
                ModelTestMongo(desc=f"test_{i}", _owner_id=user.user.id).save()
                for i in range(0, 5)
            ]
            testModels_user2 = [
                ModelTestMongo(desc=f"test_{i}", _owner_id=123456789).save()
                for i in range(5, 10)
            ]
        with self.flaskApp.test_request_context(headers={"Authorization": user.token}):
            set_user1_models = [
                set(SerializerTestMongo().dump(i).items()) for i in testModels_user1
            ]
            set_owned_entities = [
    ...
    def test_if_getOwnedEntities_do_not_return_not_owned_entities_from_db(self):
        ...
        with self.flaskApp.test_request_context(headers={"Authorization": user.token}):
            set_user2_models = [
                set(SerializerTestMongo().dump(i).items()) for i in testModels_user2
            ]
            set_owned_entities = [
                set(i.items()) for i in self.view().getOwnedEntities()
            ]
