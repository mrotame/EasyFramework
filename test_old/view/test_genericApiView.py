...
from ..testClasses import ModelTestSql, SerializerTestSql
from test.easy_framework_conftest import UserTestSql, FlaskClient
...
class TestGenericApiView:
        ...
        with Flask(__name__).test_request_context(json=self.genericJson()):
            ...
            try:
                for route in self.view.routes:
                    self.flaskApp.add_url_rule(route, view_func=self.view.as_view(f"GenericViewTest/{route}"))
    ...
    def test_if_view_auto_insert_data_into_database_with_post(self):
        with self.flaskApp.test_request_context(json=self.genericJson()):
            ...
            assert ModelTestSql.get_one(ModelTestSql.name == self.genericJson()['name']).desc == self.genericJson()['desc']

    def test_if_view_auto_read_data_from_database_with_get(self):
        with self.flaskApp.test_request_context(json=self.genericJson()):
            self.view().auto_post()

        with self.flaskApp.test_request_context(query_string={"name":self.genericJson()['name']}): 
            res = self.view().auto_get()
            assert res['desc'] == self.genericJson()['desc']
    ...
    def test_if_view_auto_update_data_from_database_with_patch(self):
        ...
        with self.flaskApp.test_request_context(query_string={"name":self.genericJson()['name']}, json= {"desc":new_desc}): 
    ...
    def test_if_getOwnedEntities_return_all_owned_entities_from_db(self):
        ...
        with self.flaskApp.app_context():
            testModels_user1 = [ModelTestSql(desc=f'test_{i}', _owner_id=user.user.id).save() for i in range(0,5)]
            testModels_user2 = [ModelTestSql(desc=f'test_{i}', _owner_id= 123456789).save() for i in range(5,10)]

        with self.flaskApp.test_request_context(headers={'Authorization': user.token}):
            set_user1_models = [set(SerializerTestSql().dump(i).items()) for i in testModels_user1]
    ...
    def test_if_getOwnedEntities_do_not_return_not_owned_entities_from_db(self):
        ...
        with self.flaskApp.test_request_context(headers={'Authorization': user.token}):
            set_user2_models = [set(SerializerTestSql().dump(i).items()) for i in testModels_user2]
            set_owned_entities = [set(i.items()) for i in self.view().getOwnedEntities()]
...
class TestGenericApiViewWithPathFieldlookup(TestGenericApiView):
    ...
    def setup(self):
        with Flask(__name__).test_request_context(json=self.genericJson()):
            self.view = self.GenericView
            self.flaskApp = FlaskClient().get_flaskApp()
    ...
    def test_if_view_auto_read_data_from_database_with_get(self):
        ...
        assert res.get_json()['id'] == entity.id
    ...
    def test_if_view_auto_update_data_from_database_with_patch(self):
        ...
        client = self.flaskApp.test_client()
        client.patch(self.GenericView.routes[0].replace("<id>",str(entity.id)), json={"desc":new_desc})

        res = client.get(self.GenericView.routes[0].replace("<id>",str(entity.id)))

        assert res.get_json()['desc'] == new_desc
